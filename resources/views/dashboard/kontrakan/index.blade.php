                        ...
                        <nav aria-label="breadcrumb" class="d-md-inline-block ml-md-4">
                            <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                                ...
                                <li class="breadcrumb-item " aria-current="page"><a
                                        href="{{ route("dashboard.kontrakan.index") }}">Kelola Kontrakan</a></li>
                            </ol>
                        </nav>
                    </div>
                </div>
                        ...
                        <div class="table-wrapper">
                            <div class="table-title">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <h2>Daftar Kontrakan
                                            <a href="{{ route("dashboard.kontrakan.create") }}" class="btn btn-success">
                                                <i class="material-icons">&#xE147;</i> <span>Tambah Kontrakan</span></a>
                                        </h2>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    @foreach ($kontrakan as $k => $hasil)
                                        <tr align="center">
                                            ...
                                            <td>
                                                ...
                                                <form action="{{ route("dashboard.kontrakan.show", $hasil->id) }}">
                                                    <button class="btn-primary"
                                                        title="Detail Data {{ $hasil->nama_kontrakan }}"
                                                        data-toggle="tooltip">
                                                        <i class="material-icons">&#xE417;</i></button>
                                                </form>
                                                <br>
                                                <br>
                                                <form action="{{ route("dashboard.kontrakan.edit", $hasil->id) }}">
                                                    <button class="btn-warning"
                                                        title="Edit Data {{ $hasil->nama_kontrakan }}"
                                                        data-toggle="tooltip">
                                                        <i class="material-icons">&#xE254;</i></button>
                                                </form>
                                                <br>
                                                <br>
                                                <form action="{{ route("dashboard.kontrakan.destroy", $hasil->id) }}"
                                                    method="POST" onsubmit="return confirm(" Yakin Hapus Data
                                                    {{ $hasil->nama_kontrakan }} ??")">
                                                    @method("delete")
                                                    @csrf
                                                    <button class="btn-danger"
                                                        title="Hapus Data {{ $hasil->nama_kontrakan }}"
                                                        data-toggle="tooltip">
                                                        <i class="material-icons">&#xE872;</i></button>
                                                </form>
                                            </td>
